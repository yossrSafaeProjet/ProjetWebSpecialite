...
router.post('/addProduct', (req, res, next) => {
   ...
   /*  if (!userId) {
    ...
    db.run(`
        INSERT INTO produits (libelle, description, images, prix, categorie, url_statistiques, userId)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [libelle, description, images, prix, categorie, url_statistiques, userId], (err) => {
        if (err) {
